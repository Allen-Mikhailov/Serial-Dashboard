		...
		switch (channelConfig.type)
		{
			case "stream":
				for (let i = 0; i < channelData.length; i++)
					total_data[key].append(channelData[i]);

				Object.keys(client_pickup_data).map((client_id: string) => {
					for (let i = 0; i < channelData.length; i++)
						client_pickup_data[client_id][key].append(channelData[i]);
				})
				break;
...
// Start the servers
listenServer.listen(configJSON.listen_port, hostname, () => {
	console.log(`Listen Server running at http://${hostname}:${configJSON.listen_port}/`);
});

// Make the server listen on the specified port and hostname
hostServer.listen(configJSON.serve_port, hostname, () => {
	console.log(`Host Server running at http://${hostname}:${configJSON.serve_port}/`);
});
